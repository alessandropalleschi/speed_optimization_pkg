...
speed_optimization_client::speed_optimization_client() 
{
  ...
  pub_command_right_ = nh_.advertise<trajectory_msgs::JointTrajectory>(joint_trajectory_command_topic_.c_str(), 1);
}
...
bool speed_optimization_client::read_trajectory(std::string fileID, trajectory_msgs::JointTrajectory &traj)
{
  ...
  //Fill the JointTrajectory Message to store the path in points.positions and the speed limits in point.velocities
